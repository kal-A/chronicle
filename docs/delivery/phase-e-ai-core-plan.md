...
| E1 | `ModelProvider` protocol, `DeterministicModelProvider`, `OllamaModelProvider`, error taxonomy, bounded retry policy, provider-billed cost recording, tested-but-unwired `AgentRunRecord`/`AgentRunStatus` scaffold | **Complete** — this document's own sub-plan; see `plans/current-phase.md` for the full write-up. No live model call has been made; no agent-run has ever actually run — both wait on E3+ and on Ollama being installed. |
| E2 | Corpus service + typed deterministic tools (`search_passages`, `get_claim_evidence`, `get_timeline_context`, etc.) over existing `GeneratedInvestigation` packages | Not started |
