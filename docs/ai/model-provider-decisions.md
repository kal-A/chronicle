...
- **Context window**: `ollama show qwen2.5:7b-instruct` reports a 32768-token architectural context length (confirmed by installing Ollama and querying it directly during the Phase E1 closeout smoke test, `docs/delivery/phase-e1-real-model-smoke-test-report.md`) — that number is the model's stated maximum, not a claim about what's safely usable given this machine's 13.69 GB RAM. The **~8K "practical" figure below remains this project's own conservative estimate**, not confirmed or refuted by that smoke test (which used a ~90-token exchange, far below either number). E2's corpus tools must still return bounded, relevant excerpts, not large unfiltered dumps, or the Planner/Analyst will silently truncate context — treat ~8K as the working assumption until a real large-context call is actually tested.
